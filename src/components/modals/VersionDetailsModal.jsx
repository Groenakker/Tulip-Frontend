...
import { FaSave, FaTrash, FaPlus, FaTimes, FaEnvelope, FaCopy, FaExternalLinkAlt, FaComment, FaDownload } from "react-icons/fa";
...
const VersionDetailsModal = ({ version, onClose, onUpdate, onDelete }) => {
  ...
  const generateApprovalToken = (stakeholder) => {
    // In production, this should be a secure token generated by the backend
    return `${version.id}-${stakeholder.id}-${Date.now()}`;
  };

  const getApprovalLink = (stakeholder) => {
    const token = generateApprovalToken(stakeholder);
    // Link will redirect to OTP verification if user is not registered
    return `${window.location.origin}/verify-otp/${token}`;
  };

  const handleCopyLink = (stakeholder) => {
    const token = generateApprovalToken(stakeholder);
    // For testing/demo: copy direct approval link
    // In production: this would be /verify-otp/{token}
    const approvalLink = `${window.location.origin}/approval/${token}`;
    navigator.clipboard.writeText(approvalLink);
    toast.success("Approval link copied to clipboard");
  };

  const handleSendApprovalEmail = async (stakeholder) => {
    const link = getApprovalLink(stakeholder);
    
    try {
      // Send approval email - backend will handle OTP generation for unregistered users
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/stakeholder-approval/send-approval-email`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
            token: generateApprovalToken(stakeholder),
            stakeholderEmail: stakeholder.email,
            stakeholderName: stakeholder.name,
            versionId: version.id,
            documentName: version.fileName || "Document",
          }),
        }
      );
      ...
      if (data.requiresOTP) {
        toast.success(`Approval link with OTP sent to ${stakeholder.email}`);
      } else {
        toast.success(`Approval email sent to ${stakeholder.email}`);
      }
    } catch (error) {
  ...
  const handleOpenApprovalLink = (stakeholder) => {
    const token = generateApprovalToken(stakeholder);
    // For testing/demo: open approval page directly
    // In production: this would go through /verify-otp/{token} first
    const approvalLink = `${window.location.origin}/approval/${token}`;
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                    ...
                    {stakeholder.status === "Pending" && (
                      <>
                        ...
                        <button
                          onClick={() => handleOpenApprovalLink(stakeholder)}
                          title="Open approval link"
                          style={{
                            padding: "6px",
                            background: "#8b5cf6",
                            color: "white",
